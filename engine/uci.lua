...
--- Builds a UCI "go" command from opts. See engine/uci_command.lua —
--- the pure implementation lives there so specs can exercise it
--- without KOReader; aliased here for callers/tests that reach it
--- through the engine module.
M.buildGoCommand = UciCommand.buildGoCommand

function UCIEngine:go(opts)
    opts = opts or {}
    local cmd = UciCommand.buildGoCommand(opts)
