...
void uci_loop(void) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        if (!strncmp(line, "ucinewgame", 10)) {
            ...
            /* slatechess patch: wipe the TT so nothing leaks between games. */
            int64_t mb = 16;
            int64_t new_size = max((int64_t)1, (mb * 1024 * 1024) / (int64_t)sizeof(TTEntry));
            TTEntry* new_tt = calloc((size_t)new_size, sizeof(TTEntry));
            if (new_tt) { free(tt); tt = new_tt; tt_size = new_size; }
        }
            ...
            int64_t movetime = 0;   /* slatechess patch: fixed per-move budget */
            ...
            getval(line, "movetime", "%" SCNd64, &movetime);

            if (movetime > 0) {
                /* slatechess patch: an explicit "go movetime" overrides the
                   clock (same precedence as Stockfish) -- without this,
                   clock mode computed a budget from the remaining time
                   (our_time/20) and searched for ~26s per move. */
